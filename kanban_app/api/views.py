...
from rest_framework.views import APIView          # Für BoardCurrentDraftView
from rest_framework.decorators import action      # Für @action submit/pdf
...
# Modelle
from kanban_app.models import Board, Task, Comment, Debriefing, GraphicsRapport
...
class BoardCurrentDraftView(APIView):
    """
    Returns the open Debriefing‑draft for today OR
    ...
    def get(self, request, pk):
        ...
        # 1) Permission‑Check (gleiche Logik wie BoardDetailView)
        user = request.user
        if not (
            user == board.owner
            or user in board.members.all()
        ...
        draft = (Debriefing.objects
                 .filter(board=board, status=Debriefing.DRAFT)
                 .order_by('-created_at')
                 .first())

        # neuer Draft, wenn keiner offen ODER Datum ungleich heute
        ...
        serializer = DebriefingSerializer(draft)
        return Response(serializer.data, status=status.HTTP_200_OK)

class DebriefingViewSet(viewsets.ModelViewSet):
    """
    CRUD‑API + Submit‑Action für SFL‑Debriefings.
    """
    queryset = Debriefing.objects.all()
    serializer_class = DebriefingSerializer
    permission_classes = []          # ← ggf. deine Auth‑Klasse eintragen

    # --------------------------------------------------------
    # 1) Draft anlegen – Erstellt wird automatisch der User
    # --------------------------------------------------------
    def perform_create(self, serializer):
        serializer.save(created_by=self.request.user)

    # --------------------------------------------------------
    # 2) Submit‑Action -> Status FINAL & PDF‑Download
    # --------------------------------------------------------
    @action(detail=True, methods=["post"])
    def submit(self, request, pk=None):
        """
        Finalisiert den Rapport und sendet das PDF als Download.
        """
        ...
        # 3. Reiner Django‑HttpResponse (kein DRF‑Renderer!)
    ...
    # --------------------------------------------------------
    # 3) Optionaler GET‑Download /pdf/
    # --------------------------------------------------------
    @action(detail=True, methods=["get"])
    def pdf(self, request, pk=None):
        """
        Lädt das PDF eines bereits eingereichten Debriefings.
        """
        deb = self.get_object()
...
class GraphicsRapportViewSet(viewsets.ModelViewSet):
    """
    CRUD‑API für Graphics‑Rapports.
    (Aktuell ohne PDF‑/Submit‑Logik – kann später analog ergänzt werden.)
    """
    queryset = GraphicsRapport.objects.all()
    serializer_class = GraphicsRapportSerializer
    permission_classes = []           # ggf. deine Auth‑Klasse einsetzen

    # User‑Feld automatisch setzen
    def perform_create(self, serializer):
        serializer.save(created_by=self.request.user)
